...
// 		}
	...
	// Universal kill streak speed bonus for all classes
	maxfbspeed *= m_Shared.GetKillStreakSpeedMod();
...
void CTFPlayer::TeamFortress_SetSpeed()
{
	...
	if ( flSpeedMod > 1.0f )
	{
		...
		Msg( "[SPEED BOOST] %s has %d kills, speed multiplier: %.2fx, final speed: %.2f\n", 
...
//-----------------------------------------------------------------------------
// Purpose: Calculate speed multiplier based on kill streak
//-----------------------------------------------------------------------------
float CTFPlayerShared::GetKillStreakSpeedMod( void ) const
{
